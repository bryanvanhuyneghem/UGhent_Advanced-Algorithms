...
BMHorspool::BMHorspool(const string &naald): naald(naald)
{
  ...
  for (int j = 0; j < naald_lengte - 1; j++)
  {
...
std::queue<int> BMHorspool::zoek(const string &hooiberg)
{
    ...
    while (shift <= (hooiberg_size - naald_size))
    {
        ...
        // mismatch
        //shift += posities[hooiberg[shift + naald_size - 1]];
        shift += naald_size - 1 - posities[hooiberg[shift + naald_size - 1]];
