...
BMVerkeerdeKarakter::BMVerkeerdeKarakter(const string &naald) : naald(naald)
{
    ...
    for (int j = 0; j < naald_lengte; j++)
    {
        ...
        //posities[naald[i]] = naald_lengte - 1 -i;
        posities[naald[j]] = j;
...
std::queue<int> BMVerkeerdeKarakter::zoek(const string &hooiberg)
{
    ...
    while(shift <= (hooiberg_size-naald_size)){
        ...
        if(naald_idx < 0){
            ...
            //hooiberg_idx++;
            //shift += 1;
            if(shift + naald_size < hooiberg_size){
                shift += naald_size - posities[hooiberg[shift + naald_size]];
            }
            else {
                shift += 1;
            }
            ...
            // Verschuif naald voorbij mismatched karakter uit hooiberg
            //std::cout << posities[hooiberg[hooiberg_idx]]  << "\n";
            //shift += posities[hooiberg[shift + naald_idx]];
            shift += std::max(1, naald_idx - posities[hooiberg[shift + naald_idx]]);
        }
