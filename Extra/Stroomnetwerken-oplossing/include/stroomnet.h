...
/**
 * @brief Vergrotendpadzoeker
 *
 * Methodeklasse die, gegeven een stroomnetwerk, een vergrotend pad teruggeeft.
 *
 * Noot: de parameters en functienamen in deze klasse zijn bewust 'obfuscated'
 * omdat het de bedoeling is dat je deze klasse als een zwarte doos beschouwt.
 * Het is niet nodig om de interne werking ervan te begrijpen.
 *
 * @tparam T
 */
template <class T>
class Vergrotendpadzoeker {
 public:
  Vergrotendpadzoeker(const Stroomnetwerk<T>& stroomnetwerk)
      : q(stroomnetwerk),
        v(stroomnetwerk.van),
        v2(stroomnetwerk.naar),
        l(q.aantalKnopen()),
        m(q.aantalKnopen(), false){};
...
void Vergrotendpadzoeker<T>::foo(int t, int x, Pad<T>& p) {
  ...
  int ychx = x + 1;
  for (typename GraafMetTakdata<GERICHT, T>::Burenlijst::const_iterator it =
           a.begin();
       it != a.end(); it++) {
    int u = it->first;
 ...
 * @brief Stroomnetwerk
 *
 * Een stroomnetwerk gaat uit van een gewogen gerichte graaf die in de
 * constructor wordt opgegeven.
 *
 * @tparam T
 */
...
class Stroomnetwerk : public GraafMetTakdata<GERICHT, T> {
 public:
  ...
  //                    Graaf<GERICHT>(gr),GraafMetTakdata<GERICHT,
  //                    T>(gr),van(0),naar(1){};
...
Stroomnetwerk<T>::Stroomnetwerk(int grootte, int _van, int _naar)
    : Graaf<GERICHT>(grootte),
      GraafMetTakdata<GERICHT, T>(grootte),
      van(_van),
      naar(_naar) {}

template <class T>
Stroomnetwerk<T>::Stroomnetwerk(const GraafMetTakdata<GERICHT, T>& gr, int _van,
                                int _naar)
    : Graaf<GERICHT>(gr),
      GraafMetTakdata<GERICHT, T>(gr),
      van(_van),
      naar(_naar) {}
...
template <class T>
Stroomnetwerk<T>::Stroomnetwerk(Stroomnetwerk<T>&& gr)
    : Stroomnetwerk(0, 0, 0) {
  ...
  for (int i = 1; i < pad.size(); i++) {
    ...
    if (terugtak != -1) {
      if (this->takdatavector[terugtak] <= nucapaciteit) {
        ...
        if (nucapaciteit > 0) vergrootTak(van, naar, nucapaciteit);
      } else {
  ...
  for (int k = 0; k < this->aantalKnopen(); k++) {
    if (this->burenlijsten[k].empty())
      ...
      for (auto& p : this->burenlijsten[k])
        uit << knooplabel(k) << " " << pijl << " " << knooplabel(p.first)
            << this->taklabel(p.second) << ";\n";
    };
  };
  uit << "}";
  uit.close();
